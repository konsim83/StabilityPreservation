...
namespace QNed
{
  ...
  QNedBasis::QNedBasis(
    const QNed::ParametersMs &                       parameters_ms,
    ...
    QNedBasis::assemble_system()
  {
    ...
    std::vector<Vector<double>> local_rhs_v(
      GeometryInfo<3>::lines_per_cell + GeometryInfo<3>::vertices_per_cell,
                ...
                for (unsigned int j = 0; j < dofs_per_cell; ++j)
                  {
                    ...
                    const Tensor<1, 3> grad_sigma_j =
                      fe_values[q1].gradient(j, q);
            ...
            for (unsigned int n_basis = 0; n_basis < length_system_basis;
                 ++n_basis)
              {
                if (n_basis < GeometryInfo<3>::vertices_per_cell)
                  {
                    // This is for h1.
                    system_rhs_h1_v[n_basis](local_dof_indices[i]) +=
                      local_rhs_v[n_basis](i);
                  }
                else
                  {
                    ...
                    const unsigned int offset_index =
                      n_basis - GeometryInfo<3>::vertices_per_cell;
                    system_rhs_curl_v[offset_index](local_dof_indices[i]) +=
                      local_rhs_v[n_basis](i);
                  }
              }
          }
    ...
    for (unsigned int n_basis = 0; n_basis < length_system_basis; ++n_basis)
      {
        ...
        if (n_basis < GeometryInfo<3>::vertices_per_cell)
          ...
          basis_ptr =
            &(basis_curl_v[n_basis - GeometryInfo<3>::vertices_per_cell]);
        ...
        if (n_basis < GeometryInfo<3>::vertices_per_cell)
        ...
        else
          {
            ...
            filename += Utilities::int_to_string(
              n_basis - GeometryInfo<3>::vertices_per_cell, 2);
          }
        filename += ".vtu";
    ...
    QNedBasis::output_global_solution_in_cell() const
  {
    ...
    std::vector<DataComponentInterpretation::DataComponentInterpretation>
      data_component_interpretation_exact(
        1, DataComponentInterpretation::component_is_scalar);
    data_component_interpretation_exact.emplace_back(
      DataComponentInterpretation::component_is_part_of_vector);
    data_component_interpretation_exact.emplace_back(
      DataComponentInterpretation::component_is_part_of_vector);
    data_component_interpretation_exact.emplace_back(
            ...
            if (n_basis < GeometryInfo<3>::vertices_per_cell)
              {
                ...
                constraints_h1_v[n_basis].condense(system_matrix,
                                                   system_rhs_h1_v[n_basis]);
            ...
            else
              {
                ...
                // Now take care of constraints
                constraints_curl_v[offset_index].condense(
                  system_matrix, system_rhs_curl_v[offset_index]);
