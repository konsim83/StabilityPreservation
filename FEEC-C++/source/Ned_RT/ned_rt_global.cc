...
namespace NedRT
{
  ...
  NedRTMultiscale::NedRTMultiscale(NedRT::ParametersMs &parameters_,
    ...
    for (; cell != endc; ++cell)
      {
        if (cell->is_locally_owned())
          {
            NedRTBasis current_cell_problem(
              parameters,
              parameter_filename,
              cell,
              first_cell,
            ...
            std::pair<typename std::map<CellId, NedRTBasis>::iterator, bool>
              result;
    ...
    NedRTMultiscale::assemble_system()
  {
    ...
    QGauss<2> face_quadrature_formula(5);
...
//	data.symmetric_operator = true; // Only for AMG
    ...
    NedRTMultiscale::run()
  {
    ...
    pcout << std::endl
          << "===========================================" << std::endl
          << "Solving >> modified Ned-RT MULTISCALE << problem in 3D."
          << std::endl;
