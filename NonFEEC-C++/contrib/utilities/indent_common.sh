...
export MsFEComplex_CLANG_FORMAT="clang-format-6.0"
...
checks() {
  ...
  if ! [ -x "$(command -v ${MsFEComplex_CLANG_FORMAT})" ]; then
    echo "***"
  ...
  CLANG_FORMAT_VERSION="$(${MsFEComplex_CLANG_FORMAT} --version)"
...
format_file()
{
  ...
  "${MsFEComplex_CLANG_FORMAT}" "${arg}" "${file}" > "${tmpfile}"
