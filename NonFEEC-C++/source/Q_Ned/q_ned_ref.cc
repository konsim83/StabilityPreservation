...
namespace QNed
{
    ...
    QNedStd::assemble_system()
  {
    ...
    const EquationData::Diffusion_A        a(parameter_filename);
        ...
        if (cell->is_locally_owned())
          {
            ...
            a.value_list(fe_values.get_quadrature_points(), a_values);
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  {
                    ...
                    const Tensor<1, 3> grad_tau_i =
                      fe_values[q1].gradient(i, q);
...
//	data.symmetric_operator = true; // Only for AMG
    ...
    QNedStd::run()
  {
    ...
    pcout << std::endl
          << "===========================================" << std::endl
          << "Solving >> Q1-Ned STANDARD << problem in 3D." << std::endl;
